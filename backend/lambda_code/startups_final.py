        ...
        if http_method == 'GET':
            if startup_id:
                ...
                if startup:
                    ...
                    return {
                        ...
                        'body': json.dumps({
                            'status': 'error',
                            'message': f'Startup com ID {startup_id} não encontrada'
                        }),
                        'headers': {
                            'Content-Type': 'application/json',
                            'Access-Control-Allow-Origin': '*'
            ...
            if not body or 'name' not in body:
                return {
                    ...
                    'body': json.dumps({
                        'status': 'error',
                        'message': 'Campo obrigatório "name" não fornecido'
                    }),
                    'headers': {
                        'Content-Type': 'application/json',
                        'Access-Control-Allow-Origin': '*'
            ...
            # Verificar se há um ID de startup
            if not startup_id:
                return {
                    'statusCode': 400,
                    'body': json.dumps({
                        'status': 'error',
                        'message': 'ID da startup não especificado'
                    }),
                    'headers': {
                        'Content-Type': 'application/json',
                        'Access-Control-Allow-Origin': '*'
            ...
            if expression_attribute_values:
                print(f"Atualizando startup com ID: {startup_id}")
                ...
                # Obter a startup atualizada
                response = startups_table.get_item(Key={'id': startup_id})
                updated_startup = response.get('Item', {})
                
                return {
                    'statusCode': 200,
                    'body': json.dumps({
                        'status': 'success',
                        'data': updated_startup
                    }),
                    'headers': {
                        'Content-Type': 'application/json',
                        'Access-Control-Allow-Origin': '*'
            ...
            # Excluir a startup
            print(f"Excluindo startup com ID: {startup_id}")
            ...
            return {
                ...
                'body': json.dumps({
                    'status': 'success',
                    'message': f'Startup com ID {startup_id} excluída com sucesso'
                }),
                'headers': {
                    'Content-Type': 'application/json',
                    'Access-Control-Allow-Origin': '*'
